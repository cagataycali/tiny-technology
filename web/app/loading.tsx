/**
 * Instant shell for the home page (`/`) while its server component resolves
 * the `no-store` worker /get fetch (up to a 10s round-trip) + the install-QR
 * build. This is the single most-visited surface, yet it was the only worker-
 * backed route WITHOUT a loading state — /[slug] and /universe both stream a
 * skeleton, so a slow worker left the landing page (but not the deep links)
 * stalling on a blank screen. The App Router streams this immediately instead.
 * Mirrors Chat's turn-zero shell (fixed header + centered hero/composer, same
 * max-w-4xl chrome) so the swap to the real page is calm, not a jump — the
 * home page renders the same <Chat> hero as /[slug], so this reuses that
 * route's skeleton verbatim. animate-pulse is neutralized under
 * prefers-reduced-motion by the global reset in globals.css.
 */
