...
describe('NiclaRecorder — a take can be stopped early', () => {
  ...
  it('sleeps in slices so the stop request is actually observed', () => {
    ...
    // The loop condition is now the pure rule (shouldExtend), not a bare deadline
    // compare: a wake take keeps running while the speaker is still talking. The
    // property this test exists for is unchanged — the loop must TICK, so a stop
    // is observed within a slice rather than at the deadline.
    expect(SRC).toMatch(/while Self\.shouldExtend\(now: Date\(\),/)
    expect(SRC).toMatch(/stopRequested: stopRequested\)/)
    // And stopRequested must still short-circuit the rule itself, or a take that
    // is being extended would ignore Stop until the speaker fell quiet.
    expect(SRC).toMatch(/if stopRequested \{ return false \}/)
  })
  ...
  it('stores the MEASURED duration, never the requested one', () => {
    // Clamped to maxSeconds, not to `clamped`: an extended wake take runs PAST
    // what was requested, so ceiling the measurement at the request would store a
    // 40-second take as a 10-second one — the same lie this test guards against,
    // mirrored, and the worse direction because the extra audio really is there.
  ...
  it('a stopped take still finalizes, transcribes, uploads and stores', () => {
    ...
    const loop = SRC.indexOf('while Self.shouldExtend(now: Date(),')
