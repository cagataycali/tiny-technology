...
# worker/src/relay.ts is the one FEATURE divergence: the missed-task report
# (relay-missed.ts + web/tests/relay-missed.test.ts) was written here first and
# upstream never took it; relay.ts carries the exports it needs on top of the
# upstream file, and index.ts schedules the sweep.
DELIBERATE='README\.md$|BETA_PIPELINE\.md$|project\.yml$|project\.pbxproj$|auto-enroll\.sh$|build-on-device\.sh$|push-ota\.sh$|ContinuityTest\.kt$|NormalizeTinySlugTest\.kt$|pay-e2e\.mjs$|settle-policy\.mjs$|wrangler\.toml$|worker/package(-lock)?\.json$|worker/config\.ts$|0029_endpoint_devices\.sql$|worker/src/index\.ts$|worker/src/relay\.ts$'
