...
| **Use your phone as a body** — buzz, torch, brightness, sounds, clipboard, alarms, screenshots, camera; every call leaves a visible trace, and a screen capture asks on the phone every single time, including when the ask came from another device | `vibrate` `flashlight` `set_brightness` `play_sound` `screenshot` `schedule_alert` `copy_to_clipboard` | [`web/lib/chat/tools/client-side.ts`](web/lib/chat/tools/client-side.ts) |
...
| **Run wearables and dev hardware** — Meta glasses, the Arduino Nicla Vision necklace, the always-listening Nicla Voice, a Flipper Zero over a cabled node *or over Bluetooth from the phone in your pocket* when that laptop is asleep (radio capture stays cable-only — BLE has no receive command) | `meta_take_photo` `meta_listen` `nicla_take_photo` `nicla_listen` `nicla_voice_wakes` `flipper_status` `flipper_files` | [`nicla.ts`](web/lib/chat/tools/nicla.ts) · [`nicla-voice.ts`](web/lib/chat/tools/nicla-voice.ts) · [`flipper.ts`](web/lib/chat/tools/flipper.ts) |
...
Under all of it: **31 D1 migrations**, **246 test files** in the web suite alone, and one
